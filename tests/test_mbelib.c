 ...
 *
 * Three branches of the radio have no counterpart in mbelib, and this decoder
 * follows the radio on all three:
 *
 ...
 *
 * The first two are frame-selective and the frames are skipped here.  The third
 * is not: it changes the spectral envelope of every frame where payload bits 40
 * and 43 differ, which is most of them, so the amplitude columns below are a
 * measurement of the gap and not a bound on this decoder's error.
 */
#include "ambe.h"
#include "testutil.h"
...
int main(void)
{
    ...
    double worst_w0 = 0, worst_gamma = 0, sum_ml = 0, worst_ml = 0;
    long ml_n = 0;
    ...
    while (fgets(bl, sizeof(bl), fb)) {
        ...
        if (reldev(t_gamma(&cur), gamma) > worst_gamma)
            worst_gamma = reldev(t_gamma(&cur), gamma);
        CHECK(worst_gamma < 5e-4, "frame %d: gamma deviation %.3e exceeds a Q11 step\n",
              n, worst_gamma);
    ...
    printf("[on the %d frames mbelib and this decoder model the same way: "
           "w0 within %.1e, gamma %.1e, L %d/%d, voicing %d/%d exact; "
           "amplitudes differ by %.1f%% mean (b3/b4); %d frames skipped, "
           "the radio's own branches] ",
           l_total, worst_w0, worst_gamma, l_same, l_total, vl_same, vl_total,
           100.0 * sum_ml / ml_n, skipped);
