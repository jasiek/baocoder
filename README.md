...
| `test_basop_firmware` | the block-float pair `Math_FloatAdd` `0x00018DD8` and `Math_FloatDivExponent` `0x00018EF4` against **the radio's own answers** rather than against libm: they approximate nothing, so there is no tolerance to quote. Edges first — exponent differences of 31, 32 and 33, both saturating operands, every sign combination | 5 checks; 3 377 cases, **all bit-exact**. Transcribing the alignment shift as a plain C `>>` instead of the machine's masked one disagrees on **257** of them |
